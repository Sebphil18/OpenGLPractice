...
void main() {
	...
	vertexOut.gTangentLightPos = vec4(tbnMatrix * lightPosition.xyz, 0);
	vertexOut.gTangentPos = tbnMatrix * vec3(worldMatrix * vec4(vPosition, 1.0));

	vec3 tangentViewPos = tbnMatrix * viewPos;
	vertexOut.gTangentViewDir = tbnMatrix * position - tangentViewPos;
	vertexOut.gTangentViewDir = normalize(vertexOut.gTangentViewDir);
