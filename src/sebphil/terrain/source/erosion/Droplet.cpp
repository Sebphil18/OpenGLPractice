...
void Droplet::erode(HeightMap& heightmap, const DropletSettings& settings) {
	for (std::uint32_t i = 0; i < settings.lifetime; i++) {
		...
		float prevHeight = getHeight(position, heightmap);

		setDirection(settings);
		position += direction;

		if (positionIsOutOfBounds(position, heightmap) || isnan(direction.x) || isnan(direction.y))
			return;

		float newHeight = getHeight(position, heightmap);
		float deltaHeight = newHeight - prevHeight;

		processSediment(heightPoint, deltaHeight, heightmap, settings);
...
bool Droplet::positionIsOutOfBounds(glm::vec2 position, const HeightMap& heightmap) {
	return position.x < 0 
		|| position.y < 0
...
float Droplet::getHeight(glm::vec2 position, const HeightMap& heightmap) {
	glm::vec2 point = glm::vec2(glm::floor(position.x), glm::floor(position.y));
	...
	gradient.x = (heightDownRight - heightDownLeft) * (1 - delta.y) + (heightUpLeft - heightUpRight) * delta.y;
	gradient.y = (heightUpLeft - heightDownLeft) * (1 - delta.x) + (heightUpRight - heightDownRight) * delta.x;

	return heightDownLeft * (1 - delta.x) * (1 - delta.y) 
		+ heightDownRight * delta.x * (1 - delta.y) + heightUpLeft * (1 - delta.x) * delta.y 
		+ heightUpRight * delta.x * delta.y;
}

// TODO: tolong
void Droplet::processSediment(const HeightMapPoint heightPoint, float deltaHeight, HeightMap& heightmap, const DropletSettings& settings) {
	...
	if (deltaHeight > 0) {
		...
		depositSediment(deposit, heightPoint, heightmap);
		...
		brush(prevPosition, erode, settings.brushRadius, heightmap);

	}
}

void Droplet::depositSediment(float deposit, const HeightMapPoint& heightPoint, HeightMap& heightmap) {
	sediment -= deposit;
...
// TODO: to long!
// float length = (delta.x * delta.x + delta.y * delta.y) / (radius * radius); maybe faster
void Droplet::brush(glm::vec2 position, float strength, float radius, HeightMap& heightmap) {
	Bounds bounds = getBoundsOfBrush(position, radius, heightmap);
	...
	for (std::uint32_t quadX = bounds.minX; quadX <= bounds.maxX; quadX++) {
		for (std::uint32_t quadY = bounds.minY; quadY <= bounds.maxY; quadY++) {
			...
			if (pointIsInCircle(delta, radius)) {
				...
				float value = (1.0f - length) * strength;
				float newHeight = heightmap.getValue(quadX, quadY) - value;

				heightmap.setValue(quadX, quadY, newHeight);
				sediment += value;
			}
...
Bounds Droplet::getBoundsOfBrush(glm::vec2 position, float radius, const HeightMap& heightmap) {
	Bounds bounds;
